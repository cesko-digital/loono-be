...
@Service
class FirebaseAuthServiceV1 : JwtAuthServiceV1 {

    private val logger = LoggerFactory.getLogger(javaClass)

    override fun verifyToken(jwt: String): JwtAuthServiceV1.VerificationResult {
        if (FirebaseApp.getApps().size == 0) {
            val firebaseOptions = loadFirebaseCredentials()
                ?: return JwtAuthServiceV1.VerificationResult.Error("Could not verify JWT.")
        ...
        return JwtAuthServiceV1.VerificationResult.Success(user)
    }
